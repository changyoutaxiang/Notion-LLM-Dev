...
class HybridCloudScheduler:
    ...
    def process_single_message(self, message):
        ...
        try:
            ...
            selected_model = message.get("model_choice", "")
            ...
            success, response = self.llm_handler.send_message(full_prompt, override_model=actual_model)
            ...
            if (self.config["settings"]["auto_generate_title"] and 
                ...
                title_success, title = self.llm_handler.generate_title(user_input)
                if title_success and title:
                    update_data["title"] = title
            
            self.notion_handler.update_message(message["page_id"], update_data)
            
            logger.info(f"✅ 消息处理完成: {message['page_id']}")
            ...
            try:
                self.notion_handler.update_message(message["page_id"], {"output": error_response})
            except Exception as update_error:
                logger.error(f"更新错误信息失败: {update_error}")
